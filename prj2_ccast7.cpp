...
void asynch_floodmax(long thread_id, int thread_num) {
    ...
    if (root) {
        mtx.lock();
        cout << "Thread num " << thread_num << " id " << thread_id << " and am ROOT." << endl;
        mtx.unlock();
    }
    else if (leaf) {
        mtx.lock();
        cout << "Thread num " << thread_num << " id " << thread_id << " and am LEAF." << endl;
        mtx.unlock();
    }
    else if (!leaf && !root) {
        mtx.lock();
        cout << "Thread num " << thread_num << " id " << thread_id << " and am NODE with parent num " << parent_num << endl;
        mtx.unlock();
    }

    // Convergecast.
    //mtx.lock();
    //cout << "Thread num " << thread_num << " starting ConvergeCast." << endl;
    //mtx.unlock();
    ...
    //mtx.lock();
    //cout << "Thread num " << thread_num << " made it past ConvergeCast." << endl;
    //mtx.unlock();

    /*
    // Empty all non-root nodes queues' to allow clear communication for next step.
    if (root != thread_num) {
        while(!q[thread_num].empty()) {
            mtx.lock();
            q[thread_num].pop();
    ...
    */
    ...
    if (root) {
        ...
        while (!q[thread_num].empty() && !children.empty()) {
    ...
    // Non-leaf nodes.
        ...
        while (q[thread_num].empty()) {                     // Wait for messages from parent.
            sleep_for(milliseconds(5));
        }
    ...
    // Leaf nodes.
        ...
        max_id = get<0>(q[thread_num].front());
    }
